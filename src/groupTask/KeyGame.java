...
public class KeyGame extends Application {
	...
	private final Button fh = new Button("返回菜单");
	...
	private Stage primateStage;

	private void initGroup(){
		group = new Group();
	    NewImage background=new NewImage();
	    Image image=new Image("image/打字游戏.png");
		background.setImage(image,626,500,30,80,group);
		initTextScore();
		initTextWrong();
		initTextTips();
		...
		initRunData();
		initTextFlyAround();
		paintHorizontalLineOnScene();
	}
	private void initTextFlyAround(){
	...
	private void initStage(Stage primaryStage){
	  ...
	  /*  Button fh=new Button("返回菜单");
	    ...
	    fh.setLayoutX(790);
	    fh.setLayoutY(220);	 
	    group1.getChildren().add(fh);
	    
		FanHui lll=new FanHui();
		lll.fanHui(fh,primaryStage);*/
